...
// ---------------------------------------------------------------------
// FUNCIÓN NUEVA: LECTURA DE PROYECTOS ARCHIVADOS
// ---------------------------------------------------------------------

function obtenerListaDeProyectosArchivados(forzar) {
  var cache = CacheService.getScriptCache();
  
  if (!forzar) {
    try {
      var chunksStr = cache.get("cache_lista_arch_chunks");
      if (chunksStr) {
        var chunks = parseInt(chunksStr, 10);
        var cachedStr = "";
        var cacheValido = true;
        for (var c = 0; c < chunks; c++) {
          var chunk = cache.get("cache_lista_arch_" + c);
  ...
  try {
    var ss = SpreadsheetApp.openById(PROYECTOS_CONFIG_ID);
    var sheet = ss.getSheetByName("BD_Proyectos_Archivados");
    if (!sheet) return []; // Si no hay archivados, retorna vacío sin romper nada
    ...
    try {
      ...
      var chunks = Math.ceil(resultStr.length / chunkSize);
      for (var i = 0; i < chunks; i++) {
        cache.put("cache_lista_arch_" + i, resultStr.slice(i * chunkSize, (i + 1) * chunkSize), 21600);
      }
      cache.put("cache_lista_arch_chunks", chunks.toString(), 21600);
    } catch(e) {}
    
    return result;
  } catch (e) {
    return { error: "Error al leer BD Archivados: " + e.message };
  }
}